...
Paper Workflow Orchestrator 是一个 Codex skill，致力于将模糊的研究想法转化为可发表的顶刊级别论文。它覆盖完整的科研到论文流程 —— 从想法打磨、文献发现、研究设计，到实验、写作、完整性审计、同行评审、修订和 AI 交接 —— 以阶段闸门和证据追踪贯穿始终。
...
以下每条特性都服务于同一个目标：让一个模糊的想法一步步接近一篇可提交的顶刊论文。每条特性都对应本仓库中的具体组件：
...
orchestrator 每个阶段路由一个主下游 skill。11 个阶段分为五个阶段组，对应从模糊想法到成稿论文的完整路径：

**阶段一 —— 构思**（将模糊的想法转化为锁定的研究问题）
